...
export const AgentDashboardLayout: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-primary-600/10 via-white to-blue-50 dark:from-gray-950/60 dark:via-gray-950/70 dark:to-gray-950/60">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        <nav className="px-2 py-4 space-y-5">
                            <Link
                                ...
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/dashboard")}`}>
                                <Home className="w-5 h-5 mr-3" /> Dashboard
                            </Link>
                            <Link
                                to='/agent/dashboard/referrals'
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/dashboard/referrals")}`}>
                                <UserPlus className="w-5 h-5 mr-3" />
                                Referrals
                            </Link>
                            <Link
                                to='/agent/dashboard/earnings'
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/dashboard/earnings")}`}>
                            ...
                            <Link
                                ...
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/dashboard/settings")}`}>
                    ...
                    <div className="flex flex-col flex-1 overflow-y-auto">
                        <nav className="px-2 py-4 space-y-5">
                            ...
                            <Link
                                to='referrals'
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/referrals")}`}>
                                <UserPlus className="w-5 h-5 mr-3" />
                                Referrals
                            </Link>
                            <Link
                                to='earnings'
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/earnings")}`}>
                                <DollarSign className="w-5 h-5 mr-3" />
                                Earnings
                            </Link>
                            <Link
                                to="settings"
                                onClick={handleNavClick}
                                className={`flex items-center px-3 py-2 text-sm font-medium rounded-md ${isActive("/agent/settings")}`}
                            >
                                <Settings className="w-5 h-5 mr-3" /> Settings
                            </Link>
                        </nav>
            ...
            {/* Main content */}
            <div className="lg:pl-64 flex flex-col flex-1 pt-2">
                {/* Top navigation */}
