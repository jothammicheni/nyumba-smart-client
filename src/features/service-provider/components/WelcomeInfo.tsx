...
import { CircleAlert, Mail, MapPin, Phone, Wrench } from 'lucide-react'
...
export default function WelcomeInfo() {
  ...
  const handleAvailability = async (userId: string, newStatus: string) => {
    ...
    try {
      ...
      // setAvailability(updatedProvider.availability)
              ...
              <div className="flex items-center">
                <div>
                  <h2 className="text-2xl font-bold leading-7 text-gray-900 dark:text-white sm:text-3xl sm:truncate">
                    Hi Welcome, {providerInfo.provider.name}
                  </h2>
                  <div className="flex flex-col mt-1 text-base text-gray-500  dark:text-gray-400">
                    ...
                    <p className="flex items-center gap-3 mt-1 text-md text-gray-500 dark:text-gray-400">
                      <Mail size='20' className='text-sm font-light' /> {providerInfo.provider.email}
                      <Phone size='20' className='text-sm font-light' /> {providerInfo.provider.phone}
                    </p>
                    <p className="flex items-center gap-3 mt-1 text-md text-gray-500 dark:text-gray-400">
                      <MapPin size='20' className='text-sm font-light' />
                      {providerInfo.provider.city}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900 dark:text-white">
                    {/* {providerInfo.provider.joinDate} */}
