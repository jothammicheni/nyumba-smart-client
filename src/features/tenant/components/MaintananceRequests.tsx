...
import { getMaintananceRequests } from "../../../services/maintananceService";
...
const MaintananceRequests = () => {
  ...
  try {
    ...
    // if (!userResponse.success || !userResponse.user?._id) {
    //   alert("Failed to fetch logged-in user.");
    //   return;
    // }

    // const tenantId = userResponse.user._id;
  ...
  <li key={request._id} className="py-3 flex items-center space-x-4">
    ...
    <div>
      ...
      >
        {request.state ? request.state.replace("-", " ").toUpperCase() : ""}
